...
def initialize_statuses():
    ...
    for num in empty_uuid_numbers:
        name = f"{num}번 컴퓨터"
        computer_statuses[name] = { 'status': 'UUID 없음', 'event_time': '-', 'last_update': datetime.now(timezone(timedelta(hours=9))).isoformat() }
...
def parse_and_update_status(message_content):
    ...
    if status:
        ...
        print(f"[상태 업데이트] {display_name}, 상태: {status}, 시간: {event_time}")
